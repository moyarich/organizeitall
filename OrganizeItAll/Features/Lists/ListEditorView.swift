...
struct ListEditorView: View {
    ...
    private func save() {
        ...
        if let list {
            ...
            modelContext.insert(TaskList(name: trimmedName, notes: trimmedNotes, createdAt: now, modifiedAt: now))
        }
        do { try modelContext.save(); dismiss() }
        catch { modelContext.rollback(); assertionFailure("Unable to save list: \(error)") }
    }
}
